...
- `pnpm lint` runs Oxlint (type-aware + type-check via `.oxlintrc.json` `options`). CI builds `react-rx` before linting so package types resolve; the website also maps `react-rx` to source via `tsconfig` paths. On GitHub Actions, Oxlint auto-selects the `github` output format.
...
- `@sanity/tsdown-config` asks for TypeScript 6 or 7 as a peer dependency. The repo stays on TypeScript 5.9 (TypeScript 7 is the native port and ships no JS API, which tsdown's `.d.ts` generation, Vitest's typechecker, and Next.js all still need), so `pnpm install` prints an unmet peer warning for it.
