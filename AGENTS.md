...
- `pnpm test` runs Vitest with `--typecheck` (type-level tests in `*.test-d.ts`) and runs each suite twice: once normally and once through the React Compiler (`react-compiler` project in `vitest.config.ts`).
