...
- `pnpm test` runs Vitest with `--typecheck` (type-level tests in `*.test-d.ts`) across the projects in `packages/react-rx/vitest.config.ts`: `default` (React 19.3), `react-compiler` (19.3 through the React Compiler), `react-19.2` (the oldest React in the peer range, aliased to the `react-19.2` / `react-dom-19.2` devDependencies; `.pnpmfile.cjs` pairs that `react-dom` with `react@19.2`) and `hermes-promise` (only the `observablePromise*` / `useObservablePromise*` suites, with React Native's species-less `Promise.prototype.then` installed by `vitest-hermes-promise.ts`). Version-dependent assertions gate on `React.version`, not the project name. Run one project with `pnpm --filter react-rx test --project react-19.2`.
