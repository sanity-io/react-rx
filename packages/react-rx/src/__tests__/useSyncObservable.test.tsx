...
test('SSR resolves a factory initialValue through getServerSnapshot', () => {
  // The sync hook's getServerSnapshot always returns the resolved initialValue — a
  // code path distinct from the client getSnapshot, which may already have an emission.
  const observable = scheduled('async value', asyncScheduler)
  function ObservableComponent() {
