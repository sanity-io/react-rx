 ...
 * `getSnapshot` returns the provided `initialValue` until the store subscription (started on
 * commit) delivers the first emission into the entry's state. The hooks resolve initializer
 * functions once (like `useState`) and pass that stable value in — `getSnapshot` must not
 * re-invoke them, or an object-returning factory would fail `Object.is` on every read and
