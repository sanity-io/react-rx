...
- [#627](https://github.com/sanity-io/react-rx/pull/627) [`64f6fb1`](https://github.com/sanity-io/react-rx/commit/64f6fb1bf6195f56a6553bddd87b3d99b56f7ff7) Thanks [@breken-ai](https://github.com/breken-ai)! - Fix `useObservablePromise` and `preloadObservablePromise` on Hermes (React Native), where creating the promise crashed with `Maximum call stack size exceeded`. Hermes does not implement `Symbol.species`, so `.then()` on a `Promise` subclass built another instrumented instance and recursed in the constructor. The returned `ObservablePromise` no longer subclasses `Promise`: it is a thenable that wraps a plain promise and delegates `then` / `catch` / `finally` to it, so derived promises are plain `Promise`s on every engine and `use(promise.then(fn))` settles instead of suspending forever. It still satisfies the `ObservablePromise<T>` type and works with `use()`, `await` and the `Promise` combinators, but `instanceof Promise` is now `false`.

## 7.0.0

### Major Changes

- [#515](https://github.com/sanity-io/react-rx/pull/515) [`8ca73bd`](https://github.com/sanity-io/react-rx/commit/8ca73bd0426197639813131626df41c5be26f964) Thanks [@stipsan](https://github.com/stipsan)! - v7 moves every subscription out of the render phase and removes `useObservableEvent`. Requirements are unchanged from v6: React `^19.2`, RxJS `^7.2`, Node `>=22.12`, ESM only. The [v6 to v7 migration guide](https://react-rx.dev/migrate/v6-to-v7) has more patterns.
...
- [#585](https://github.com/sanity-io/react-rx/pull/585) [`8fed462`](https://github.com/sanity-io/react-rx/commit/8fed4628da88aa15de0d5c06a1736c499b327c81) Thanks [@stipsan](https://github.com/stipsan)! - Add `useObservableSubject`, a hook that returns the two halves of an event: an observable of the values pushed into it, and a referentially stable handler that pushes them. The underlying RxJS `Subject` is scoped to the component instance and only its observable side is exposed.
