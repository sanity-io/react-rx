...
// …is one hook. Lifecycle, teardown, SSR, deferred updates: handled.
...
- **First-class [`<Activity>`](https://react.dev/reference/react/Activity) (React 19.2).** Hidden trees keep warm snapshots and reveal instantly. Own a promise in a visible component and the hidden tree pre-renders with the data through `use()`.
- **SSR you can predict.** `useObservable` renders exactly what the client's first paint will show, and nothing is ever subscribed on the server. `useSyncObservable` keeps the strict server-snapshot contract when you want it.
...
| `<Activity>` pre-rendering (React 19.2) | Hidden trees pre-render with `use(promise)`; `preloadObservablePromise` for hover warm-up | No                                                  | No                                                    | No                              |
| Snapshot on mount, remount, reveal      | Latest value of an already-live stream, else `initialValue`                               | `useObservableEagerState`                           | With an active subscription                           | Extra render after mount        |
...
| SSR                                     | Server renders `initialValue`, matching the client's first paint                          | Renders initial state                               | No                                                    | Manual                          |
...
- [Migration guides](https://react-rx.dev/migrate/v6-to-v7)
