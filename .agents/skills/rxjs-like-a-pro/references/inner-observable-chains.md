...
function showNotification(message: string) {
  return concat(
    ...
    animateOpacity(0, 1, 300).pipe(
      map(opacity => ({ message, opacity, visible: true })),
    ),
    // Phase 2: stay visible for 3 seconds
    timer(3000).pipe(
      map(() => ({ message, opacity: 1, visible: true })),
    ),
    // Phase 3: fade out over 300ms
    animateOpacity(1, 0, 300).pipe(
      map(opacity => ({ message, opacity, visible: opacity > 0 })),
    ),
  );
}
...
notification$.pipe(
  switchMap(message => showNotification(message)),
