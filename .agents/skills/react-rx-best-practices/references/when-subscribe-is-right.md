...
this list _before_ rewriting them.

## Leave alone: subscriptions that aren't value reads

- **Event-driven stores.** Code that subscribes to listen/mutation/connection events and _dispatches_
...
- **Eager, non-cancellable promise producers.** If building the "observable" means calling something
  like `attempt(fetch(...))` — an API that _fires immediately_ and can't be cancelled — then creating
...
- **Write-side reads.** A value read synchronously in an event handler to decide _whether to write_
...
- Did lines of code go _up_ without deleting a state machine?
