...
## Related: `initialValue` is not a stability tool
...
- Lazy `initialValue` functions are safe: like `useState`, the initializer is resolved once per hook
  instance and never re-run on later renders. (On v6 and earlier it ran on every pre-emission
  snapshot read, so a function returning a fresh object each call looped with "The result of
  getSnapshot should be cached" / "Maximum update depth exceeded".)
- The identity contract is stricter since v7 precisely _because_ there is no render-phase warm-up
  anymore: a fresh identity always renders the `initialValue` and re-subscribes at commit, so an
  observable rebuilt every render over a synchronously-replaying source loops forever.
- `disabled: true` performs no subscriptions at all, so it is a valid way to gate an observable with
  subscribe-time side effects. Swapping the observable (`shouldFetch ? fetch$ : of(null)`) is still
  fine when the "off" state should render a specific value.
