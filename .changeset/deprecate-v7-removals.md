...
- Calling `useObservable` or `useSyncObservable` without an `initialValue` is removed in v7, where the argument is required. `useObservable(observable$, undefined)` keeps the v6 type and behavior.
