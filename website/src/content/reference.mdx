...
This is the building block [`useObservableEvent`](#useobservableevent) is made of, and the recommended replacement for it: `useObservableEvent` is removed in v7 while `useObservableSubject` carries over unchanged, so call sites migrated to it need no further changes. See the [v6 → v7 migration guide](/migrate/v6-to-v7#useobservableevent-is-removed).
...
> [!WARNING]
>
> Deprecated. v7 removes `useObservableEvent`. Use [`useObservableSubject`](#useobservablesubject)
> instead, as shown in [Handling events](/guide#handling-events). See the [v6 to v7 migration
> guide](/migrate/v6-to-v7#useobservableevent-is-removed).
