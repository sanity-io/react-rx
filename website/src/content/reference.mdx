...
The hook does **not** suspend, and rendering never subscribes the source: the fetch starts when a component that called the hook commits, or via [`preloadObservablePromise`](#preloadobservablepromise). Call the hook in a component that does not itself suspend and pass the returned promise to React's [`use`](https://react.dev/reference/react/use) in a child inside a `<Suspense>` boundary — a component that suspends on its own promise never commits, so it can never start its own fetch. Suspends until the first emission; later emissions update without re-suspending. Errors reject the promise (Error Boundary). Hidden `<Activity>` trees calling the hook stay paused until revealed. See the [guide](/guide) for `startWith` caveats, `disabled` / `ttl`, Activity patterns, and when to prefer `useObservable`.
