...
> [!NOTE]
>
> v7 is in prerelease. Install it with `npm install react-rx@next`. Its docs live at
> [react-rx-git-next.sanity.dev](https://react-rx-git-next.sanity.dev). v6 marks every API this
> guide removes as `@deprecated`, so editors flag call sites ahead of the upgrade. `no-deprecated`
> lint rules also flag uncalled references such as `typeof useObservable`, because they read the
> tags of every overload. Those are false positives. Suppress them locally.
