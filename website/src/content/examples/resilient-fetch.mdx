...
- **Errors don't kill the poll** — `catchError` sits on the *inner* request observable, so an exhausted retry becomes a status value while the outer timer keeps ticking.
